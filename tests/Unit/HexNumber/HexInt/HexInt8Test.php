...
class HexInt8Test extends TestCase
{
    ...
    public function testInt8($hex, $int)
    {
        ...
        $message = 'HexInt8::fromHex($hex)->toUnPrefixed() convert hex: ' . $hex . ' into expected int: ' . $int;
        $this->assertEquals($hex, HexInt8::fromHex($hex)->toUnPrefixed(), $message);

        $message = 'HexInt8::fromHex($hex)->toPrefixed() convert hex: ' . $hex . ' into expected int: ' . $int;
        ...
        $this->assertEquals($hex, HexConverter::intToHex($int, HexInt8::HEX_LENGTH), $message);
